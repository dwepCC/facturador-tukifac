...
class RestaurantServiceProvider extends ServiceProvider
{
    ...
    {
        $viewPath = resource_path('views/modules/restaurant');
        ...
        $this->loadViewsFrom(array_merge(array_map(function ($path) {
            return $path . '/modules/restaurant';
        }, \Config::get('view.paths')), [$sourcePath]), 'restaurant');
    }
    ...
    {
        $langPath = resource_path('lang/modules/restaurant');

        if (is_dir($langPath)) {
            $this->loadTranslationsFrom($langPath, 'restaurant');
        } else {
