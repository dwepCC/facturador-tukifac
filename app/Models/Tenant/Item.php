...
class Item extends ModelTenant
{
     ...
     * @return HasMany
     */
    public function warehouse_prices()
    {
 ...
 *
    ...
    public function getDataToItemModal(
        ...
        $data = [
            ...
            'date_of_due' => !empty($this->date_of_due) ? $this->date_of_due->format('Y-m-d') : null,
    ...
    {
        $this->attributes['unit_type_id'] = str_replace(' ', '',$value);
    }

    public function setCurrencyTypeIdAttribute($value)
    {
        $this->attributes['currency_type_id'] = str_replace(' ', '',$value);
    }
    ...
    public function scopeWhereFilterRecordsApi($query, $input, $search_by_barcode)
    {
        ...
        $query->where(function ($q) use ($keywords, $input) {
            foreach ($keywords as $word) {
                $q->where('description', 'like', "%{$word}%");
            }
            $q->orWhere('internal_id', 'like', "%{$input}%");
        });
