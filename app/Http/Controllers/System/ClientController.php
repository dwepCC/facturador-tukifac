...
public function records(Request $request)
{
    ...
    foreach ($records as &$row) {
        ...
        $current_month_start = $current_day->copy()->startOfMonth()->format('Y-m-d');
        $current_month_end = $current_day->copy()->endOfMonth()->format('Y-m-d');
        
        // Aplicar rango de fechas exclusivo para documentos si está presente
        $documents_query = DB::connection('tenant')->table('documents');
        if ($documents_date_start && $documents_date_end) {
            $documents_query->whereBetween('date_of_issue', [$documents_date_start, $documents_date_end]);
        } else {
            $documents_query->whereBetween('date_of_issue', [$current_month_start, $current_month_end]);
        }
        
        $row->current_count_doc_month = $documents_query->count();
        
        $row->count_doc_pse = DB::connection('tenant')
            ->table('documents')
        ...
        if ($row->start_billing_cycle) {
            ...
            // CRÍTICO: Usar copy() o clonar para evitar mutación
            $init = $start_end_date['start_date'];
            $end = $start_end_date['end_date'];
            
            // Asegurar que tenemos las fechas en formato string para la consulta
            $init_formatted = $init instanceof Carbon ? $init->format('Y-m-d') : $init;
            $end_formatted = $end instanceof Carbon ? $end->format('Y-m-d') : $end;

            $client_helper = new ClientHelper();

            // Contar documentos en el rango del ciclo de facturación
            // Si hay rango de fechas exclusivo para documentos, aplicarlo
            $documents_cycle_query = DB::connection('tenant')->table('documents');
            if ($documents_date_start && $documents_date_end) {
                $documents_cycle_query->whereBetween('date_of_issue', [$documents_date_start, $documents_date_end]);
            } else {
                $documents_cycle_query->whereBetween('date_of_issue', [$init_formatted, $end_formatted]);
            }
            $row->count_doc_month = $documents_cycle_query->count();
            
            $row->sale_notes_quantity_if_include = 0;

            // Si el plan incluye sale notes en el límite de documentos
            if ($row->plan->includeSaleNotesLimitDocuments()) {
                // Aplicar rango de fechas exclusivo para sale_notes si está presente
                if ($documents_date_start && $documents_date_end) {
                    $row->sale_notes_quantity_if_include = $client_helper->getQuantitySaleNotesByDates(
            ...
            // Contar sale notes en el rango del ciclo de facturación
            // Si hay rango de fechas exclusivo para documentos, aplicarlo también a sale_notes
            $sale_notes_query = DB::connection('tenant')->table('sale_notes');
            if ($documents_date_start && $documents_date_end) {
                $sale_notes_query->whereBetween('date_of_issue', [$documents_date_start, $documents_date_end]);
            } else {
                $sale_notes_query->whereBetween('date_of_issue', [$init_formatted, $end_formatted]);
            }
            $row->count_sales_notes_month = $sale_notes_query->count();

            // Actualizar quantity_sales_notes si cambió
            ...
            // Recargar el valor actualizado
            $row->count_sales_notes = DB::connection('tenant')
                ->table('configurations')
                ->first()
                ->quantity_sales_notes;

            // Calcular total de ventas mensuales (sin aplicar el rango de documentos)
            $row->monthly_sales_total = $client_helper->getSalesTotal(
                $init_formatted, 
                $end_formatted, 
                $row->plan
            );
